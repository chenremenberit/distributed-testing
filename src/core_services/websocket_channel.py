...
class WebSocketChannel:
    ...
    async def websocket_server_start_info(self):
        """
        接收从客户端发来的消息并处理，再回复客户端。
        """
        self.logger.info("websocket_server_start")
    ...
    async def start_websocket_server(self):
        ...
        启动服务器进程保持一直运行
        """
        async with websockets.serve(self.websocket_server_start_info(), self.host, self.port):
